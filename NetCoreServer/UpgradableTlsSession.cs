...
namespace NetCoreServer
{
    ...
    public class UpgradableTlsSession 
        : IDisposable
    {
        /// <summary>
        ...
        public UpgradableTlsSession(UpgradableTlsServer server)
        {
            Id = Guid.NewGuid();
            Server = server;
        ...
        /// Server
        /// </summary>
        public UpgradableTlsServer Server { get; }
        /// <summary>
        /// Socket
        /// </summary>
        ...
        private System.IO.Stream _sslStream;
        ...
        internal void Connect(Socket socket)
        {
            ...
            try
            {
                ...
                _sslStream = new NetworkStream(Socket, false);
        ...
        public virtual void Upgrade()
        {
            ...
            try
            {
                // Create SSL stream
                _sslStreamId = Guid.NewGuid();
                SslStream sslConnection = (Server.Context.CertificateValidationCallback != null) ? new SslStream(new NetworkStream(Socket, false), false, Server.Context.CertificateValidationCallback) : new SslStream(new NetworkStream(Socket, false), false);


                // Begin the SSL handshake
                sslConnection.BeginAuthenticateAsServer(Server.Context.Certificate, Server.Context.ClientCertificateRequired, Server.Context.Protocols, false, ProcessHandshake, _sslStreamId);

                _sslStream = sslConnection;
            }
            catch (Exception)
            {
                SendError(SocketError.NotConnected);
        ...
        public virtual bool Disconnect()
        {
            ...
            try
            {
                try
                {
                    ...
                    if (_sslStream is NetworkStream)
                    {
                        NetworkStream a = (NetworkStream)_sslStream;
                        a.Close();
                    }
                    else if (_sslStream is SslStream)
                    {
                        SslStream a = (SslStream)_sslStream;
                        a.ShutdownAsyncExt().Wait();
                    }
                }
                catch (Exception) {}
        ...
        private void ProcessHandshake(IAsyncResult result)
        {
            try
            {
                ...
                if (_sslStream is SslStream)
                {
                    SslStream a = (SslStream)_sslStream;
                    a.EndAuthenticateAsServer(result);
                }
                else
                    throw new System.InvalidOperationException("Cannot end authentication o non-ssl-stream.");
