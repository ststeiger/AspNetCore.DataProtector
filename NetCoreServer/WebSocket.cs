        ...
        public void PrepareReceiveFrame(byte[] buffer, long offset, long size)
        {
            lock (WsReceiveLock)
            {
                ...
                while (size > 0)
                {
                    ...
                    WsReceiveBuffer.AddRange(NetCoreServer.RuntimeHelpers.GetSubArray(buffer, new Range((int)offset + index, (int)offset + index + length)));
